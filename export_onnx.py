...
# Define your constants if not already defined
def export_onnx():
    model = SparseAutoencoder_Conv(input_size=(1, 128, 768), latent_channels=1, num_classes=6)
    model.eval()

    # Create dummy input (match the expected input shape)
...
class SparseInputMask(nn.Module):
    def __init__(self, height, width,n_ch, k=1, tau=1.0):
        """
...
class SparseAutoencoder_Conv(nn.Module):
    ...
    def sparse_reconstruction_loss(self, P, logits, targets,P_mean_d,start_po_loss):
        ...
        total_loss = cls_loss + batch_p0_loss
...
def training_loop(model, dataloader,targets_names, optimizer, scheduler, num_class, epochs=10, device='cpu', run_name='default'):
    ...
    for epoch in tqdm(range(epochs)):
        ...
        for input_tensor, targets, indices in dataloader:
            ...
            loss= model.sparse_reconstruction_loss( P, logits, targets,P0_mean_l,accuracy_history[-1] >0.6 if len(accuracy_history)>0 else False )
            loss.backward()
            optimizer.step()
            scheduler.step()
    ...
    for cls in dataloader.dataset.tensors[1].unique():
        ...
        flat_samples = samples.view(samples.shape[0], -1)  # [B, H*W]
        clusteres[f"label:{cls}"] = flat_samples[:, indices.detach().cpu()]
...
if __name__ == '__main__':
    ...
    wanted_model="CONV"
    run_names=['best_neurons_activation_only','one_mask_for_all_grads_only','one_mask_for_all_activation_and_gards']
    epochs=500
    num_class=6
    ACTIVE_NEURONS_COUNTS=[1,2,10,100,500,1000]
    WANTED_LAYERS=[0, 5,-1]
    CFG = load_config(config_name='config')
    path_to_pkls=[[r'saved_activations/activations_dict.pkl'],[r'saved_activations/grad_activations_dict.pkl'],[r'saved_activations/activations_dict.pkl',r'saved_activations/activations_dict.pkl']]
